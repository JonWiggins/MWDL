<?php include("../includes/header.php"); ?>
<meta name="description" content="University of Utah - American West Center, a Partner of the MWDL">
<?php include("../includes/linkImports.php");?>
<script type="text/javascript">
	if (screen.width <= 800) {
  		window.location = "../amppartners/UniversityofUtahAmericanWestCenter.php";
	}
</script>
<!-- Mobile Links -->
<link rel="amphtml" href="../amppartners/UniversityofUtahAmericanWestCenter.php">
<link rel="alternate" media="only screen and (max-width: 640px)" href="../amppartner/UniversityofUtahAmericanWestCenter.php">
<!-- Title -->
<title> University of Utah - American West Center </title>
<?php include("../includes/partnermenuhead.php");?>
<div class="imageAndDes">
<!-- Image -->
<img src="../images/partner_images/partner140.jpg" alt=""width="392" height="53"align= "right" style="max-width: 250px; height: auto; margin: 1%; display: block; ">
<!-- Image Description -->

</div>
<!-- Partner #140 -->
<!-- Partner Name -->
<h3>University of Utah - American West Center</h3>
<!-- Website Link -->
<h6><a href="http://www.awc.utah.edu/">University of Utah - American West Center Website </a></h6>
<!-- Article Text -->
<p><br/><p>In 1964 A. R. Mortensen and C. Gregory Crampton, two University of Utah history professors, founded the American West Center with the then unique mission of researching the history and culture of the American West. Over the last forty-three years the Center has accomplished far more than Mortensen and Crampton could have imagined. Faculty and graduate student researchers affiliated with the Center have taken over 7,000 oral histories, including an unparalleled 2,000 Native American interviews. Center staff have also created a remarkable archive of documents essential to the study of the Western environment and produced myriad curriculum support materials--most notably seventeen community history textbooks--for underserved communities across the region.</p></p>
<hr>
<a href="http://utah-primoprod.hosted.exlibrisgroup.com/primo_library/libweb/action/search.do?tab=default_tab&mode=Advanced&scp.scps=scope:(mw)&vid=MWDL&indx=1&dum=true&srt=rank&frbg=&fn=search&ct=search&vl(1UI1)=exact&vl(35820410UI1)=lsr12&vl(freeText1)=University+of+Utah+-+American+West+Center">Browse all records from University of Utah - American West Center</a>
<!-- List all Active Collections -->
<li><a href = "../collections/UtahAmericanIndianDigitalArchiveOralHistoriesTranscripts.php">Utah American Indian Digital Archive: Oral Histories Transcripts</a></li>
<li><a href = "../collections/UtahAmericanIndianDigitalArchive.php">Utah American Indian Digital Archive</a></li>
</div>
</div>
<?php include("../includes/footer.php"); ?>
